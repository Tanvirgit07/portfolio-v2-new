...
const StackedCard = ({
  ...
  // API থেকে আসা প্রথম ইমেজটি নেওয়া হচ্ছে
      ...
      >
        <div className="flex-1 p-10 md:p-14 flex flex-col justify-between relative z-10 bg-[#0f100a]">
          <div>
            ...
            <p className="text-slate-400 text-sm md:text-base leading-relaxed mb-8 max-w-sm line-clamp-3">
              {/* API description array থেকে প্রথম লাইনটি নেওয়া হচ্ছে */}
...
const GridCard = ({ project, onOpen }: { project: any; onOpen: (p: any) => void }) => {
  const projectImage = project.images?.[0]?.url || "";
  const displayDesc = Array.isArray(project.description) ? project.description[0] : project.description;
    ...
    <section className="relative bg-[#0f100a] px-6 min-h-screen selection:bg-[#c7d300] selection:text-black">
      ...
      <div className="max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          ...
          {view === "grid" && (
            <motion.div key="grid" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mt-10">
              {projects.map((project: any) => (
                <GridCard key={project._id} project={project} onOpen={handleOpenModal} />
              ))}
            </motion.div>
          )}

          {view === "list" && (
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="mt-10 flex flex-col bg-[#15160e]/50 rounded-[40px] border border-white/10 overflow-hidden">
              {projects.map((project: any, i: number) => (
                <ListCard key={project._id} index={i} project={project} onOpen={handleOpenModal} />
              ))}
            </motion.div>
          )}
        </AnimatePresence>
                    ...
                    <div className="flex flex-col h-full">
                      <div className="flex-1 relative m-6 md:m-10 rounded-[2.5rem] overflow-hidden border border-neutral-800">
                        {selectedProject.images?.[0]?.url && (
                          <Image fill src={selectedProject.images[0].url} alt="Main" className="object-cover" />
                        )}
                      </div>
                    </div>
                </div>
                   ...
                   <div className="p-8 md:p-16 space-y-12">
                      ...
                      <div className="space-y-8">
                        {Array.isArray(selectedProject.description) ? (
                          selectedProject.description.map((desc: string, i: number) => (
                            <p key={i} className="text-xl md:text-2xl text-neutral-300 leading-relaxed font-light border-l-4 border-[#c7d300] pl-8">{desc}</p>
                          ))
                        ) : (
                      ...
                      {selectedProject.logicSnippet && (
                        <div className="space-y-4">
                          ...
                          <pre className="bg-black p-8 rounded-3xl border border-neutral-800 overflow-x-auto text-sm font-mono text-[#c7d300]/90 scrollbar-hide"><code>{selectedProject.logicSnippet}</code></pre>
                        </div>
                      )}
                      ...
                      <a 
                        ...
                        target="_blank" 
                        className="w-full py-6 bg-[#c7d300] text-black font-black uppercase text-sm flex items-center justify-center gap-3 rounded-[2rem] hover:bg-white transition-all shadow-[0_10px_30px_rgba(199,211,0,0.2)]"
                      >
