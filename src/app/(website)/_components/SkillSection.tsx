      ...
      {visible && (
        ...
        >
          ...
          >
            ...
            <p className="text-slate-300 text-[12px] leading-relaxed mb-3">{skill.description}</p>
...
function SkillCard({ skill }: { skill: Skill }) {
  ...
  return (
    ...
    >
      ...
      <div className={`relative z-10 w-16 h-16 transition-all duration-500 ${hovered ? "scale-110 -translate-y-2" : "scale-100"}`}>
        <Image 
          src={skill.image} 
          alt={skill.name} 
          fill 
...
export default function SkillSection() {
  ...
  const { data: response, isLoading } = useQuery({
    queryKey: ["skills"],
    queryFn: async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_API_URL}/skill/getallSkill`); // আপনার API URL দিন
      return res.json();
    },
  });
  ...
  if (isLoading) return <div className="min-h-[800px] bg-[#15160e]" />;
